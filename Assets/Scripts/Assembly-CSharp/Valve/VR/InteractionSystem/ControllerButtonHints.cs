...
namespace Valve.VR.InteractionSystem
{
	public class ControllerButtonHints : MonoBehaviour
	{
				...
				if (instance != null)
				{
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							componentButtonMasks.Add(new KeyValuePair<string, ulong>(transform.name, componentButtonMask));
							string text2 = text;
							text = text2 + "\n\t" + transform.name + ": " + componentButtonMask;
						}
					}
					finally
					{
		...
		private void HideAllText()
		{
			foreach (KeyValuePair<EVRButtonId, ButtonHintInfo> buttonHintInfo in buttonHintInfos)
			{
				buttonHintInfo.Value.textHintObject.SetActive(false);
